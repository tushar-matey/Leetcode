...
public:
    ...
    bool isInterleave(string s1, string s2, string s3) {
        ...
        if(s1[s1.size()-1]==s3[s3.size()-1] && s2[s2.size()-1]==s3[s3.size()-1]){
            dp[s1.size()][s2.size()-1]=true;
            dp[s1.size()-1][s2.size()]=true;
            dp[s1.size()][s2.size()]=true;

        }
        else if(s1[s1.size()-1]==s3[s3.size()-1]){
            dp[s1.size()][s2.size()-1]=true;
        }
        else if(s2[s2.size()-1]==s3[s3.size()-1]){
            dp[s1.size()-1][s2.size()]=true;
        }
        else{
            return false;
